...
Module UtilLogFile
    ...
    Public Sub CreateLog(Optional ByVal cLogFileName As String = "WinTexTiciMaxLog", Optional ByVal cLogMessage As String = "")
        Try
            Dim cTodaysDate As String = String.Format("{0:dd_MM_yyyy}", DateTime.Now)
            Dim cFileName As String = "C:\wintex\WinTexTiciMaxLogs\" + cLogFileName + "_" + cTodaysDate + ".txt"
            Dim cMessage As String = ""

            cMessage = vbCrLf +
                        "-------------------------------------------------------" + vbCrLf +
                        "WinTexTiciMax versiyon : " + HTMain.cWinTexTiciMaxVersion + vbCrLf +
                        "Tarih / Saat : " + Now.ToString + " " + vbCrLf +
                        "Log : " + cLogMessage.Trim + vbCrLf

            If (My.Computer.FileSystem.DirectoryExists("C:\wintex\WinTexTiciMaxLogs") = False) Then
                My.Computer.FileSystem.CreateDirectory("C:\wintex\WinTexTiciMaxLogs")
            End If
    ...
    Public Sub ErrDisp(Optional ByVal cErrorMessage As String = "", Optional ByVal cFormName As String = "", Optional ByVal cSQL As String = "", Optional ByVal lShowMessage As Boolean = False, Optional oEx As Exception = Nothing)
        Try
            ...
            cMessage = "WinTexTiciMax versiyon : " + HTMain.cWinTexTiciMaxVersion + vbCrLf +
                        "Tarih / Saat : " + Now.ToString + " " + vbCrLf +
            ...
            If (My.Computer.FileSystem.DirectoryExists("C:\wintex\WinTexTiciMaxErrors") = False) Then
                My.Computer.FileSystem.CreateDirectory("C:\wintex\WinTexTiciMaxErrors")
            End If

            File.AppendAllText("C:\wintex\WinTexTiciMaxErrors\WinTexTiciMaxError" + "_" + cTodaysDate + ".txt",
                                   vbCrLf +
                                   "-------------------------------------------------------" + vbCrLf +
                                   cMessage + vbCrLf)
