...
Module UtilSQL
    ...
    Public Function GetSQLQuery(nCase As Integer, Optional cFilter As String = "") As String
        ...
        Try
            Select Case nCase
                Case 1
                    GetSQLQuery = "select b.Secim, a.StokNo, b.CinsAciklamasi, a.Renk, a.Beden, " +
                            " SatilabilirAdet = coalesce(giren,0) - coalesce(cikan,0) " +
                            " from satilabilirstok() a , stok b with (NOLOCK) " +
                            " where a.stokno = b.stokno " +
                            " and a.stokno is not null " +
                            " and a.stokno <> '' " +
                            cFilter
                Case 2
                    GetSQLQuery = "select distinct a.StokNo, b.CinsAciklamasi, a.Renk, a.Beden, a.TiciMaxid, a.TiciMaxVaryasyonid, a.TiciMaxUpdate, a.SatisFiyati2 " +
                            " from stokrb2 a With (NOLOCK) , stok b With (NOLOCK) " +
                            " where a.stokno = b.stokno " +
                            " and a.ticimax = 'E' " +
                            " and a.stokno is not null " +
                            " and a.stokno <> '' " +
                            " and a.renk is not null " +
                            " and a.renk <> '' " +
                            " and a.beden is not null " +
                            " and a.beden <> '' " +
                            cFilter +
                            " order by a.StokNo, a.Renk "
                Case 3
                    GetSQLQuery = "select siparisno, tarih, adi, soyadi, ili, " +
                            " ilcesi, telefon, adres, kargofirmasi, kargostatu, " +
                            " finaltutar, ticimaxid, ticimaxupdate " +
                            " from sipperakende with (NOLOCK) " +
                            " where ticimaxid is not null " +
                            " and ticimaxid <> 0 " +
                            " and (kapandi is null or kapandi = '' or kapandi = 'H') " +
        ...
        Catch ex As Exception
            ErrDisp("GetSQLQuery", "utilSQL",,, ex)
        End Try
    End Function
End Module
