...
' Dikkat bu Class bu dll için özeldir 
...
Public Class SQLServerClass
    ...
    Public Sub New(Optional lDefault As Boolean = True, Optional cServer As String = "", Optional cDatabase As String = "", Optional cUsername As String = "", Optional cPassword As String = "")
        ...
        If lDefault Then
            cConnectionString = "Data Source=" + utilMain.oConnection.cServer + ";" +
    ...
    Public Sub init(Optional nCase As Integer = 1)
        ...
        'Select Case utilMain.oConnection.cOwner

        '    Case "vera"
        '        cConnectionString = "Data Source=192.168.1.216;" +
        '                            "Initial Catalog=veragroup;" +
        '                            "uid=sa;" +
        '                            "pwd=wintex"
        '    Case "vera_local"
        '        cConnectionString = "Data Source=monster;" +
        '                            "Initial Catalog=veragroup;" +
        '                            "uid=sa;" +
        '                            "pwd=Hayabusa1024"
        '    Case "eroglu"
        '        Select Case nCase
        '            Case 1
        '                cConnectionString = "Data Source=192.168.1.8;" +
        '                                    "Initial Catalog=tes;" +
        '                                    "uid=sa;" +
        '                                    "pwd=er1303*?"
        '            Case 2
        '                cConnectionString = "Data Source=192.168.1.8;" +
        '                                    "Initial Catalog=MISIR;" +
        '                                    "uid=sa;" +
        '                                    "pwd=er1303*?"
        '        End Select
    ...
    Public Function CLRExecute(ByVal cProcName As String, Optional ByVal cFilter As String = "", Optional cFilter2 As String = "", Optional cFilter3 As String = "",
                               Optional cFilter4 As String = "", Optional ByRef cMessage As String = "") As Boolean
        ...
        Try
            ...
            Select Case cProcName
                ...
                Case "UretimisEmriUretCLR"
                    ...
                    ' 5
                    oParam = New SqlParameter With {
                        .ParameterName = "cDefaultPersonel",
                        .SqlDbType = SqlDbType.Char,
                        .Direction = ParameterDirection.Input,
                        .Size = 1,
                        .Value = ""
                ...
                Case "RPASiparis2", "FastUTFBuildAll", "FastMTFBuildAll", "HizliStokBakimi"
                ' no parameters
                ...
                Case "dobeforeadd_rpasiparis", "doafteradd_rpasiparis"

                    oParam = New SqlParameter With {
                        .ParameterName = "cSiparisNo",
                        .SqlDbType = SqlDbType.Char,
                        .Direction = ParameterDirection.Input,
                        .Size = Len(cFilter) + 1,
                        .Value = cFilter
            ...
            Select Case cProcName
                ...
                Case "UretimisEmriUretCLR"
                    If CInt(CmdAdo.Parameters.Item(10).Value) = 1 Then
                        CLRExecute = True
                    End If
                ...
                Case "dobeforeadd_rpasiparis", "doafteradd_rpasiparis"
                    ...
                    cMessage = CmdAdo.Parameters.Item(2).Value.ToString
        ...
        Catch ex As Exception
            ErrDisp("CLRExecute", "SQLServerClass",,, ex)
        End Try
    End Function
    ...
    Public Function GetDataTableFromView(cViewName As String) As DataTable

        GetDataTableFromView = Nothing

        Try
            Dim oDS As New DataSet()
            Dim oDataAdapter As SqlDataAdapter

            cSQLQuery = "select * from " + cViewName
            oDataAdapter = New SqlDataAdapter(cSQLQuery, oConnection)
            oDataAdapter.SelectCommand.CommandTimeout = 600
            oDataAdapter.Fill(oDS, cViewName)
            GetDataTableFromView = oDS.Tables(cViewName)

        Catch ex As Exception
            ErrDisp("GetDataTableFromView", "SQLServerClass", cSQLQuery,, ex)
        End Try
    End Function
    ...
    Public Function GetFisNo(ByVal cKeyField As String, Optional ByVal cFormat As String = "") As String
        ...
        Try
            ...
            cSQLQuery = "select parametervalue " +
                        " from syspar with (NOLOCK) " +
                        " where parametername = '" + cKeyField + "' "

            If CheckExists() Then
                nFisNo = CDbl(DBReadString())
                nFisNo = nFisNo + 1
                If cFormat = "" Then
                ...
                cSQLQuery = "update syspar " +
                            " set parametervalue = " + nFisNo.ToString +
                            " where parametername = '" + cKeyField + "' "
                SQLExecute()
            Else
                cSQLQuery = "insert into syspar (parametername, parametervalue) " +
        ...
        Catch ex As Exception
            ErrDisp("GetFisNo : " + cKeyField.Trim, "UtilSQL", cSQLQuery,, ex)
        End Try
    End Function

    Public Sub SaveDevXReport(cReportName As String, cReportClass As String, aPLF As Byte(), aPGF As Byte(), aPCF As Byte(), aCF As Byte())
        ...
        Try
            ...
            cSQLQuery = "insert into devxreports (reportid, reportname, reportclass)  " +
                        " values ('" + cRecID.Trim + "', " +
                        " '" + SQLWriteString(cReportName.Trim) + "', " +
                        " '" + SQLWriteString(cReportClass.Trim) + "') "
            SQLExecute()

            cSQLQuery = "update devxreports set fileplf = @plf, filepcf = @pcf, filepgf = @pgf, filecf = @cf " +
                        " where reportname = '" + cReportName.Trim + "' " +
                        " and reportclass = '" + cReportClass.Trim + "' "

            oCMD = New SqlCommand(cSQLQuery, oConnection)
        ...
        Catch ex As Exception
            ErrDisp(ex.Message, "SaveDevXReport", cSQLQuery,, ex)
        End Try
    End Sub
    ...
    Public Sub FillDataTable()
        ...
        Catch ex As Exception
            ErrDisp(ex.Message, "FillDataTable", cSQLQuery,, ex)
        End Try
    End Sub

    Public Function GetDataTable(cTableName As String) As DataTable
        ...
        Try
            ...
            oDataAdapter = New SqlDataAdapter(cSQLQuery, oConnection)
            oDataAdapter.SelectCommand.CommandTimeout = 600
            oDataAdapter.Fill(oDS, cTableName)
            GetDataTable = oDS.Tables(cTableName)

        Catch ex As Exception
            ErrDisp("GetDataTable", "SQLServerClass", cSQLQuery,, ex)
        End Try
    End Function
