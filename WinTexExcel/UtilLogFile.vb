...
Module UtilLogFile
    ...
    Public Sub CreateLog(Optional ByVal cLogFileName As String = "WinTexExcelLog", Optional ByVal cLogMessage As String = "")
        Try
            Dim cTodaysDate As String = String.Format("{0:dd_MM_yyyy}", DateTime.Now)
            Dim cFileName As String = "C:\wintex\WinTexExcelLogs\" + cLogFileName + "_" + cTodaysDate + ".txt"
            Dim cMessage As String = ""

            cMessage = vbCrLf +
                        "-------------------------------------------------------" + vbCrLf +
                        "WinTexExcel versiyon : " + Assembly.GetExecutingAssembly().GetName().Version.ToString + vbCrLf +
                        "Tarih / Saat : " + Now.ToString + " " + vbCrLf +
                        "Log : " + cLogMessage.Trim + vbCrLf

            Console.WriteLine(cMessage)

            If (My.Computer.FileSystem.DirectoryExists("C:\wintex\WinTexExcelLogs") = False) Then
                My.Computer.FileSystem.CreateDirectory("C:\wintex\WinTexExcelLogs")
            End If
    ...
    Public Sub ErrDisp(Optional ByVal cErrorMessage As String = "", Optional ByVal cFormName As String = "", Optional ByVal cSQL As String = "", Optional ByVal lShowMessage As Boolean = False, Optional oEx As Exception = Nothing)
        Try
            ...
            cMessage = "WinTexExcel versiyon : " + Assembly.GetExecutingAssembly().GetName().Version.ToString + vbCrLf +
                        "Tarih / Saat : " + Now.ToString + " " + vbCrLf +
            ...
            Debug.WriteLine("WinTexExcel Hata : " + cMessage)
            ...
            If (My.Computer.FileSystem.DirectoryExists("C:\wintex\WinTexExcelErrors") = False) Then
                My.Computer.FileSystem.CreateDirectory("C:\wintex\WinTexExcelErrors")
            End If

            File.AppendAllText("C:\wintex\WinTexExcelErrors\WinTexExcelError" + "_" + cTodaysDate + ".txt",
                                   vbCrLf +
                                   "-------------------------------------------------------" + vbCrLf +
                                   cMessage + vbCrLf)
