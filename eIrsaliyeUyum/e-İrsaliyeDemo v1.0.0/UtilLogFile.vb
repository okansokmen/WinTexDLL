    ...
    Module UtilLogFile
        ...
        Public Sub AddEventLog(cMessage As String, Optional nCase As Integer = 1)
            Try
                ...
                If Not System.Diagnostics.EventLog.SourceExists("WinTexUyum") Then
                    System.Diagnostics.EventLog.CreateEventSource("WinTexUyum", "WinTexUyumLog")
                End If

                oEventLog.Source = "WinTexUyum"
                oEventLog.Log = "WinTexUyumLog"

                cMessage = "WinTexUyum versiyon : " + My.Application.Info.Version.ToString.Trim + vbCrLf +
                       "Server : " + UtilRoot.oConnection.cServer.Trim + " " + vbCrLf +
                       "Database : " + UtilRoot.oConnection.cDatabase.Trim + " " + vbCrLf +
                       "User : " + UtilRoot.oConnection.cUser.Trim + " " + vbCrLf +
        ...
        Public Sub CreateLog(Optional ByVal cLogFileName As String = "WinTexUyumLog", Optional ByVal cLogMessage As String = "")
            Try
                Dim cTodaysDate As String = String.Format("{0:dd_MM_yyyy}", DateTime.Now)
                Dim cFileName As String = "C:\wintex\WinTexUyumLogs\" + cLogFileName + "_" + cTodaysDate + ".txt"
                ...
                Debug.WriteLine("WinTexUyum Log : " + cMessage)

                'If oConnection.lWinTexDllLog Then
                If (My.Computer.FileSystem.DirectoryExists("C:\wintex\WinTexUyumLogs") = False) Then
                    My.Computer.FileSystem.CreateDirectory("C:\wintex\WinTexUyumLogs")
                End If
        ...
        Public Sub ErrDisp(Optional ByVal cErrorMessage As String = "", Optional ByVal cFormName As String = "", Optional ByVal cSQL As String = "", Optional ByVal lShowMessage As Boolean = False, Optional oEx As Exception = Nothing)
            Try
                Dim cTodaysDate As String = String.Format("{0:dd_MM_yyyy}", DateTime.Now)
                Dim cFileName As String = "C:\wintex\WinTexUyumError\WinTexUyumError" + "_" + cTodaysDate + ".txt"
                Dim cMessage As String = ""
                ...
                Debug.WriteLine("WinTexUyum Hata : " + cMessage)

                If lShowMessage Or lGlobalDebugMode Then
                    MsgBox("WinTexUyum Hata : " + cMessage)
                End If

                If InStr(cMessage, "404") = 0 Then

                    If (My.Computer.FileSystem.DirectoryExists("C:\wintex\WinTexUyumError") = False) Then
                        My.Computer.FileSystem.CreateDirectory("C:\wintex\WinTexUyumError")
                    End If
