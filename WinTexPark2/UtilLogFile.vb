...
Module UtilLogFile
    ...
    Public Sub CreateLog(Optional ByVal cLogFileName As String = "WinTexParkLog", Optional ByVal cLogMessage As String = "")
        Try
            Dim cTodaysDate As String = String.Format("{0:dd_MM_yyyy}", DateTime.Now)
            Dim cFileName As String = "C:\wintex\WinTexParkLogs\" + cLogFileName + "_" + cTodaysDate + ".txt"
            Dim cMessage As String = ""

            cMessage = vbCrLf +
                        "-------------------------------------------------------" + vbCrLf +
                        "WinTexPark versiyon : " + Assembly.GetExecutingAssembly().GetName().Version.ToString + vbCrLf +
            ...
            If (My.Computer.FileSystem.DirectoryExists("C:\wintex\WinTexParkLogs") = False) Then
                My.Computer.FileSystem.CreateDirectory("C:\wintex\WinTexParkLogs")
            End If
    ...
    Public Sub ErrDisp(Optional ByVal cErrorMessage As String = "", Optional ByVal cFormName As String = "", Optional ByVal cSQL As String = "", Optional ByVal lShowMessage As Boolean = False, Optional oEx As Exception = Nothing)
        Try
            ...
            cMessage = "WinTexPark versiyon : " + Assembly.GetExecutingAssembly().GetName().Version.ToString + vbCrLf +
                        "Tarih / Saat : " + Now.ToString + " " + vbCrLf +
            ...
            Debug.WriteLine("WinTexPark Hata : " + cMessage)

            MsgBox(cMessage)

            'Console.WriteLine(cMessage)

            If (My.Computer.FileSystem.DirectoryExists("C:\wintex\WinTexParkErrors") = False) Then
                My.Computer.FileSystem.CreateDirectory("C:\wintex\WinTexParkErrors")
            End If

            File.AppendAllText("C:\wintex\WinTexParkErrors\WinTexParkError" + "_" + cTodaysDate + ".txt",
                                   vbCrLf +
                                   "-------------------------------------------------------" + vbCrLf +
                                   cMessage + vbCrLf)
