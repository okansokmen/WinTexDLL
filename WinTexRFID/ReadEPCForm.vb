	...
	Partial Public Class ReadEPCForm
		...
		Private Sub ReadEPC()
			Try
				...
				'                    if (info != null)
				'                    {
				'                        this.BeginInvoke(setTextCallback2, new object[] { info.Epc, info.Tid, info.Rssi, "1", info.Ant, info.User });
				'                    }
				'                    else
				'                    {
				'                        if (isRuning)
...
'ORIGINAL LINE: label6.Text = (tempCount += int.Parse(count)).ToString();
			...
			Else
				...
				Dim itemTid As New ListViewItem.ListViewSubItem()
				itemTid.Name = "TID"
				itemTid.Text = tid
				lv.SubItems.Add(itemTid)

				Dim itemUser As New ListViewItem.ListViewSubItem()
				itemUser.Name = "USER"
				itemUser.Text = user
				...
				epcList.Insert(index, New EpcInfo(epc, Integer.Parse(count), DataConvert.HexStringToByteArray(epc), DataConvert.HexStringToByteArray(tid)))
		...
		Private Sub btnExport_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnExport.Click
			Dim path As String = Environment.CurrentDirectory & "\uhfData" & DateTime.Now.ToString("yyyy_MM_dd_HHssmm") & ".xls"
			ExcelUtils.ExportExcels(path, lvEPC)
		End Sub

		Private Sub rb2s_Click(ByVal sender As Object, ByVal e As EventArgs) Handles rb2s.Click
			txtTime.Text = "2"
		End Sub

		Private Sub rb3s_Click(ByVal sender As Object, ByVal e As EventArgs) Handles rb3s.Click
			txtTime.Text = "3"
		End Sub

		Private Sub rb4s_Click(ByVal sender As Object, ByVal e As EventArgs) Handles rb4s.Click
			txtTime.Text = "4"
		End Sub

		Private Sub rb5s_Click(ByVal sender As Object, ByVal e As EventArgs) Handles rb5s.Click
			txtTime.Text = "5"
		End Sub

		Private Sub rb10_Click(ByVal sender As Object, ByVal e As EventArgs) Handles rb10.Click
			txtTime.Text = "10"
		End Sub

		Private Sub test()
			Dim time As Integer = 0
			txtTime.Invoke(New EventHandler(Sub()
				gbAuto.Enabled = False
				Try
					time = Integer.Parse(txtTime.Text)*1000
				Catch ex As Exception
				End Try
			End Sub))
			If time = 0 Then
				time = Integer.MaxValue
			End If
			...
			Do
				...
				Else
					...
					End If
				End If
				If Environment.TickCount - beginTime >= time Then
					If Not isStop Then
						isStop = True
						uhf.StopGet()
						t1 = Environment.TickCount
					Else
						If Environment.TickCount - t1 > 100 Then
							isRuning = False
							Exit Do
						End If
					End If
				...
				txtTime.Invoke(New EventHandler(Sub()
					Dim time2 As Long = Environment.TickCount - beginTime
					If time2 > time Then
						time2 = time
					End If
					lblTime.Text = (time2) & "ms"
