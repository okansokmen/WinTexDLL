    ...
    public partial class ReadEPCForm : BaseForm
    {
        ...
        private void UpdataEPC(string epc, string tid, string rssi, string count,string ant,string user)
        {
            ...
            if (exist[0])
            {
                ...
                epcList.Insert(index, new EpcInfo(epc, int.Parse(count), DataConvert.HexStringToByteArray(epc), DataConvert.HexStringToByteArray(tid)));
        ...
        private void test()
        {
            ...
            txtTime.Invoke(new EventHandler(delegate
            {
                ...
                try { time = int.Parse(txtTime.Text)*1000; }
                catch (Exception ex) { }
            }));
            if (time == 0)
            {
            ...
            while (true)
            {
                ...
                if (System.Environment.TickCount - beginTime >= time)
                {
                    if (!isStop)
                    {
                        isStop = true;
                    ...
                    {
                        if (System.Environment.TickCount - t1 > 100)
                        {
                            isRuning = false;
                            break;
                        }
                ...
                txtTime.Invoke(new EventHandler(delegate
                {
                    long time2 = System.Environment.TickCount - beginTime;
                    if (time2 > time)
                    {
                        time2 = time;
                    }
