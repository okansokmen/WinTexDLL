...
Module UtilLogFile
    ...
    Public Sub CreateLog(Optional ByVal cLogFileName As String = "WinTexRFIDLog", Optional ByVal cLogMessage As String = "")
        Try
            Dim cTodaysDate As String = String.Format("{0:dd_MM_yyyy}", DateTime.Now)
            Dim cFileName As String = "C:\wintex\WinTexRFIDLogs\" + cLogFileName + "_" + cTodaysDate + ".txt"
            Dim cMessage As String = ""


            cMessage = vbCrLf +
                        "-------------------------------------------------------" + vbCrLf +
                        "WinTexRFID versiyon : " + My.Application.Info.Version.ToString.Trim + vbCrLf +
                        "Tarih / Saat : " + Now.ToString + " " + vbCrLf +
                        "Log : " + cLogMessage.Trim + vbCrLf

            If (My.Computer.FileSystem.DirectoryExists("C:\wintex\WinTexRFIDLogs") = False) Then
                My.Computer.FileSystem.CreateDirectory("C:\wintex\WinTexRFIDLogs")
            End If
    ...
    Public Sub ErrDisp(Optional ByVal cErrorMessage As String = "", Optional ByVal cFormName As String = "", Optional ByVal cSQL As String = "", Optional ByVal lShowMessage As Boolean = False, Optional oEx As Exception = Nothing)
        Try
            ...
            cMessage = "WinTexRFID versiyon : " + My.Application.Info.Version.ToString.Trim + vbCrLf +
                        "Tarih / Saat : " + Now.ToString + " " + vbCrLf +
            ...
            If (My.Computer.FileSystem.DirectoryExists("C:\wintex\WinTexRFIDErrors") = False) Then
                My.Computer.FileSystem.CreateDirectory("C:\wintex\WinTexRFIDErrors")
            End If

            File.AppendAllText("C:\wintex\WinTexRFIDErrors\WinTexRFIDError" + "_" + cTodaysDate + ".txt",
                                   vbCrLf +
                                   "-------------------------------------------------------" + vbCrLf +
                                   cMessage + vbCrLf)
