	...
	Partial Public Class UHFUpgradeFormEx
		...
		Private Sub btnStart_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnStart.Click
			...
			If path Is Nothing OrElse path.Length = 0 Then
				...
				Return
			End If
			If Not rbReaderApplication.Checked AndAlso Not rbUHFModule.Checked AndAlso Not rbReaderBootloader.Checked AndAlso Not rbEx10SDKFirmware.Checked Then
				MessageBox.Show("fail")
				Return
			End If
		...
		Private Sub startUpdate()
			...
			Try
				...
				Me.Invoke(New EventHandler(Sub()
					If rbReaderApplication.Checked Then
						type = 0
						strversion = "uhf version:" & uhf.GetSTM32Version()
						Me.Invoke(New EventHandler(Sub()
							label2.Text = strversion
						End Sub))
					ElseIf rbUHFModule.Checked Then
						type = 1
						strversion = "uhf version:" & uhf.GetSoftwareVersion()
						Me.Invoke(New EventHandler(Sub()
							label2.Text = strversion
						End Sub))
					ElseIf rbReaderBootloader.Checked Then
						type = 2
					ElseIf rbEx10SDKFirmware.Checked Then
						type = 3
					End If
				End Sub))
		...
		Private Sub getVersion(ByVal type As Byte)
